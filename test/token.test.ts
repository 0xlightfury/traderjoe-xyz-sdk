...
describe('Token', () => {
  ...
  describe('#equals', () => {
    ...
    it('false if chain id differs', () => {
      expect(new Token(ChainId.ARBITRUM, ADDRESS_ONE, 18).equals(new Token(ChainId.AVALANCHE, ADDRESS_ONE, 18))).toBe(
        false
      )
    })
    ...
    it('true even if name/symbol/decimals differ', () => {
      const tokenA = new Token(ChainId.AVALANCHE, ADDRESS_ONE, 9, 'abc', 'def')
      const tokenB = new Token(ChainId.AVALANCHE, ADDRESS_ONE, 18, 'ghi', 'jkl')
